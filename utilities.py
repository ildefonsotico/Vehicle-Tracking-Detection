...
def get_hog_features_2(img, orient, pix_per_cell, cell_per_block,
    ...
    if vis == True:
        ...
        if (verbose):
            plt.imshow(img)
            plt.savefig(out_path + 'original_'+name+'.png')
            plt.imshow(hog_image, cmap='gray')
            plt.savefig(out_path + 'hog_'+name+'.png')
            ...
            if hog_channel == 'ALL':
                ...
                for channel in range(feature_image.shape[2]):
                    hog_features.append(get_hog_features(feature_image[:, :, channel],
                                                         ...
                                                         vis=False, feature_vec=True, verbose=False))
                hog_features = np.ravel(hog_features)
            else:
                hog_features = get_hog_features(feature_image[:, :, hog_channel], orient,
                                                pix_per_cell, cell_per_block, vis=False, feature_vec=True, verbose=False)
